...
$Self->{'Ticket::Frontend::AgentTicketPhoneOutbound'}->{'RichTextHeight'} =  '320';
$Self->{'Ticket::Frontend::AgentTicketPhoneOutbound'}->{'RichTextWidth'} =  '620';
$Self->{'Ticket::Frontend::AgentTicketPhoneInbound'}->{'RichTextHeight'} =  '320';
...
$Self->{'CustomerDBLink'} =  '$Env{"CGIHandle"}?Action=AgentCustomerInformationCenter;CustomerID=$QData{"CustomerID"}';
...
$Self->{'Frontend::Module'}->{'AdminProcessManagement'} =  {
  ...
  'Loader' => {
    ...
    'JavaScript' => [
      'thirdparty/jsplumb-1.4.2/jsplumb.js',
      'Core.Agent.Admin.ProcessManagement.js',
...
$Self->{'Frontend::Search'}->{'AgentCustomerInformationCenter'} =  {
  '^AgentCustomerInformationCenter' => 'Action=AgentCustomerInformationCenter'
};
...
$Self->{'Frontend::RichTextPath'} =  '<OTRS_CONFIG_Frontend::WebPath>js/thirdparty/ckeditor-4.0/';
...
$Self->{'DefaultUsedLanguages'} =  {
  ...
  'sl' => "Slovenian (Sloven\x{161}\x{10d}ina)",
